...
export default function TicketInfoLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Check if user has ticket_info role
        if (user.role !== 'ticket_info') {
          console.log('❌ Wrong role:', user.role);
          setError('Access denied. Only Ticket Info users can login here.');
          setLoading(false);
          return;
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-green-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Ticket Info Display</h1>
          <p className="text-gray-600">Login to access ticket information screen</p>
        </div>
        ...
        <div className="text-center mt-6 text-gray-600 text-sm">
          <p>login for Ticket Info Display users only</p>
        </div>
      </div>
    </div>
  );
